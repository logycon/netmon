...
func DeviceCapture(deviceName string, filter string, out chan Packet) {
...
func packetToMessage(packet gopacket.Packet) Packet {
	v := Packet{}
	...
	if Contains(v.Layers, "arp") {
		if arpLayer := packet.Layer(layers.LayerTypeARP); arpLayer != nil {
			arp, _ := arpLayer.(*layers.ARP)

			v.ARP = &ARP{
				Operation: IIF(arp.Operation == layers.ARPRequest, "ArpRequest", "ArpReply"),
				Sender: ARPParticipant{
					MAC: net.HardwareAddr(arp.SourceHwAddress).String(),
					IP:  net.IP(arp.SourceProtAddress).String(),
				},
				Target: ARPParticipant{
